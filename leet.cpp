...
std::vector<std::vector<int>> combinationSum(std::vector<int>& candidates, int target) {
  ...
  std::unordered_map<int, std::array<int, 4>> combinations;
  ...
  for (size_t number = 0; number < candidates.size(); number++) {
    ...
    while (sum + candidates[number] <= target) {
      ...
      for (int divisor = 1; divisor <= sqrt(difference); divisor++) {
      ...
      if (combinations.count(difference) && (!visited.count({difference, combinations[difference][0]})) && (!visited.count({difference, combinations[difference][1]})) && (!visited.count({combinations[difference][0], combinations[difference][1]})) && (!visited.count({combinations[difference][1], combinations[difference][0]}))) {
        visited.insert({difference, combinations[difference][0]});
        visited.insert({difference, combinations[difference][1]});
        visited.insert({combinations[difference][0], combinations[difference][1]});
        while (count > 0) {
          current.push_back(candidates[number]);
          count--;
        }
