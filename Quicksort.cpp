...
std::vector<int>& Quicksort::sortIterative(std::vector<int>& list) {
  ...
  struct quickSortCall {
    // List is not required as we have a pointer to it already and each call works on the same list
    int left;
    int right;
  };
  ...
  while (!callStack.empty()) {
    ...
    // If 'base case' is reached, pop the current stack 'frame' and continue to next iteration
    if (left >= right) {
      callStack.pop();
      continue;
    }
